...
ALL_MODULES = [
    "moderation", "automod", "logging", "utility", "fun",
    "economy", "leveling", "tickets", "giveaways", "welcome",
    "roles", "music", "starboard", "birthdays"
]
...
class Config(commands.Cog):
    ...
    @commands.hybrid_group(name="config", description="Server configuration and feature toggles.",
    ...
    async def config_group(self, ctx: commands.Context) -> None:
        ...
        for mod in ALL_MODULES:
            ...
            lines.append(f"• **{mod.capitalize()}:** {'✅' if enabled else '❌'}")

        settings = g.get("settings", {})
        if settings:
            lines.append("\n**Configured Channels & Keys:**")
            for k, v in list(settings.items())[:10]:
                val_str = f"<#{v}>" if str(k).endswith("_channel") else f"`{v}`"
                lines.append(f"• `{k}`: {val_str}")

        await ctx.send(embed=embeds.titled(
            await self.bot.tr(ctx.guild.id, "config_title"), "\n".join(lines)))

    @config_group.command(name="prefix", description="Set a custom command prefix for this server.")
    @is_admin()
    async def config_prefix(self, ctx: commands.Context, prefix: str) -> None:
        if len(prefix) > 10:
            return await ctx.send(embed=embeds.error("Prefix is too long (maximum 10 characters)."))
    ...
    @config_group.command(name="modules", description="Enable or disable a feature module.")
    @is_admin()
    async def config_modules(self, ctx: commands.Context, module: str, enabled: bool) -> None:
        ...
        if module not in ALL_MODULES:
            return await ctx.send(embed=embeds.error(
                f"Unknown module. Available modules:\n{', '.join(ALL_MODULES)}"))
        await db.set_module(ctx.guild.id, module, enabled)
        await ctx.send(embed=embeds.success(await self.bot.tr(
            ctx.guild.id, "config_updated", key=f"modules.{module}", value=str(enabled))))

    @config_group.command(name="set", description="Set a channel setting (welcome_channel, starboard_channel, etc.).")
    @is_mod()
    async def config_set(self, ctx: commands.Context, key: str,
                         channel: discord.TextChannel) -> None:
        allowed = {
            "welcome_channel", "leave_channel", "suggestion_channel",
            "message_log_channel", "member_log_channel", "voice_log_channel",
            "server_log_channel", "mod_log_channel", "ticket_log_channel",
            "starboard_channel", "birthday_channel", "count_channel"
        }
